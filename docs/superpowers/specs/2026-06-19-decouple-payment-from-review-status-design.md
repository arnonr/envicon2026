...
- เพิ่ม column "สถานะการชำระเงิน" ใน worksheet (header key: `paymentStatus`, width: 20)
- เพิ่ม `PAYMENT_STATUS_NAMES` map (unpaid=ยังไม่ชำระ, pending_verification=รอตรวจสอบ, verified=ชำระแล้ว, rejected=ปฏิเสธ)
- Bump autoFilter range จาก `A1:T1` → `A1:U1` (20 → 21 columns)
...
ไฟล์นี้มี diff ค้างอยู่ (filter author info ออกจาก reviewer view) — รวม commit เดียวกัน: extend pattern `if (isAdmin) { response.paymentStatus = ...; } else { delete response.paymentStatus; }` (เพิ่ม `paymentStatus` ใน select ด้านบน แล้ว delete ใน reviewer branch).

### Admin release decision (`backend/src/routes/admin-reviews.ts:521`)

`POST /admin/review-rounds/:roundId/release` ไม่เปลี่ยน logic (ไม่เช็ค paymentStatus) — เพราะ user กำหนดให้ admin เป็น gatekeeper. Warning "ผลงานนี้ยังไม่ชำระเงิน" เป็น frontend concern เท่านั้น.
...
- เพิ่ม Payment section ที่แสดงตอน `paymentStatus !== 'verified'` (ใช้ได้ทุก status ของ submission รวมทั้ง draft — ผู้ส่งเห็น QR ได้ตั้งแต่ยังไม่อัปโหลด abstract):
...
### E. `pages/admin/index.vue` (admin list) + `components/admin/SubmissionDetailModal.vue` (admin modal)

- `Submission` interface (ทั้ง 2 ไฟล์): เพิ่ม `paymentStatus: string`
- ใน `pages/admin/index.vue`:
...
- ใน `components/admin/SubmissionDetailModal.vue` (admin modal):
  - เพิ่ม section "การชำระเงิน" แสดง `PaymentStatusBadge` + `paymentSlipUrl` link (ถ้ามี) + `paymentNote` (ถ้า rejected)
  - เพิ่มปุ่ม "✓ ยืนยันการชำระเงิน" / "✗ ปฏิเสธ" (visible เฉพาะ `paymentStatus === 'pending_verification'`)
    - กด verify → `PATCH .../payment` body `{ paymentStatus: "verified" }`
    - กด reject → เปิด UModal เล็กให้กรอก note (required) → `PATCH .../payment` body `{ paymentStatus: "rejected", note }`
...
| สถานการณ์ | พฤติกรรม |
|---|---|
| Author จ่ายตอน `status=draft` (ก่อนอัปโหลด abstract) | API อนุญาต `upload-slip` ทุก status ที่ paymentStatus≠verified; paymentStatus=pending_verification. Status ยังคง draft. UI แสดง payment section ใน modal ตามปกติ (Section 3C) |
...
| `frontend/pages/admin/index.vue` | Filter + column ใน submission list |
| `frontend/components/admin/SubmissionDetailModal.vue` | เพิ่ม payment section + verify/reject buttons |
